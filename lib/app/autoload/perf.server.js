...
YUI.add('mojito-perf', function (Y, NAME) {
    ...
    Y.namespace('mojito.perf').applyConfig = function(config) {
        perfConfig = config;
    
        Y.mojito.perf.idFromCommand = perfConfig ? idFromCommand : function () {};

        Y.mojito.perf.instrumentMojitoRequest = perfConfig ? instrumentMojitoRequest : function () { console.log('====MISSED instrumentMojitoRequest'); };

        Y.mojito.perf.timeline = perfConfig.timeline ? timeline : function () {
            return {
                done: function () {}
            };
        };

        Y.mojito.perf.mark = perfConfig.mark ? mark : function () {};

        Y.mojito.perf.dump = perfConfig ? dump : function () {};
    };

    Y.mojito.perf.applyConfig(Y.config.perf || {});
